...
class AutocompleteSystem {
    ...
    PriorityQueue<Pair<String, Integer>> PQ = new PriorityQueue<>((a, b) -> {
            Integer c1 = a.getValue();
            Integer c2 = b.getValue();
    ...
    void insert(Trie t, String sen, int count) {
        for (char c : sen.toCharArray()) {
            int ind = toInt(c);            
            if (t.child[ind] == null) t.child[ind] = new Trie();
            t = t.child[ind];
        }
        t.times += count;
    }
    ...
    void lookup(Trie t, List<String> result, String s) {
        LinkedList<String> result2 = new LinkedList<>();
        for (int i = 0; i < s.length(); i++) {
            if (t.child[toInt(s.charAt(i))] == null) return;
            t = t.child[toInt(s.charAt(i))];
        }        
        dfs(t, s);
        while (!PQ.isEmpty()) {
            result2.addFirst(PQ.remove().getKey());
        }
    ...
    void dfs(Trie t, String curr) {
        if (t.times > 0) addToPQ(curr, t.times);
        for (int i = 0; i < 26; i++) {
            if (t.child[i] != null) dfs(t.child[i], curr + (char) ('a' + i));            
        }
        if (t.child[26] != null) dfs(t.child[26], curr + " ");
    }
    
    void addToPQ(String str, int count) {
        PQ.add(new Pair(str, count));
    ...
    public List<String> input(char c) {
        ...
        if (c == '#') {
            insert(root, currSen, 1);
            currSen = "";
        } else {
            currSen += c;
            lookup(root, result, currSen);
        }
        return result;
    }
}
