...
class Solution {
    public boolean isInterleave(String s1, String s2, String s3) {
        ...
        for (int i = 1; i <= n1; i++) {
            for (int j = 1; j <= n2; j++) {
                if (s1.charAt(i - 1) == s3.charAt(i + j - 1)) dp[i][j] = dp[i - 1][j]; // if last char of s1 and s3 mathced.
                if (s2.charAt(j - 1) == s3.charAt(i + j - 1) && !dp[i][j]) dp[i][j] = dp[i][j - 1]; 
                // if last char of s1 and s3 matched but dp[i][j] is till false because dp[i - 1][j] was false, and 
                // now last char of s2 and s3 is also same. Let's give dp[i][j] one more try and check if it gets equal.
            }
        }
...
class Solution {
    public boolean isInterleave(String s1, String s2, String s3) {
        int n1 = s1.length(), n2 = s2.length(), n3 = s3.length();
        if (n1 + n2 != n3) return false;
        boolean[] dp = new boolean[n2 + 1]; // can take n1 also.
        dp[0] = true;
        for (int i = 0, j = 1; j <= n2; j++) {
            if (s2.charAt(j - 1) == s3.charAt(j - 1) && dp[j - 1]) dp[j] = true;
        }
        for (int i = 1; i <= n1; i++) {
